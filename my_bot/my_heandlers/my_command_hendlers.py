...
async def send_weather(is_send_message:bool = False):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get('http://127.0.0.1:8000/browser_page_screen_shot', params = {'file_name': 'test'}, timeout = 30) as response:
...
def send_traffic(is_send_message:bool = False):
    global bot
    ...
    try:
        # browser_page_screen_shot(file_name=file_name, url=url, click_type=ClickType.traffick_click_1)
        r = req.get('http://127.0.0.1:8000/browser_page_screen_shot', timeout=60, params = {'url': url, 'click_type':ClickType.traffick_click_1.value})
        with open(file_name, 'wb') as file:
...
# def send_evening_newsletter_in_chat(is_send_message:bool = False):
#     global bot
